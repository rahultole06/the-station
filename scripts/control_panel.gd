...
@onready var activate_door_label: RichTextLabel = %ActivateDoorLabel
@onready var head_down_label: RichTextLabel = %HeadDownLabel
...
func _process(_delta: float) -> void:
	...
	if (clickable && Input.is_action_just_pressed("interact")):
		activate_noise.play() # noise for door
		
		# objective updation
		activate_door_label.hide()
		head_down_label.show()
